...
#' Y =  X + M %*% b + rnorm(N)
#' sparse.mediation.tau(X,M,Y,tol=10^(-10),max.iter=100,lambda = log(1+(1:25)/50))
...
sparse.mediation = function(X,M,Y,tol=10^(-10),max.iter=100,
  ...
  for (j in 1:length(alpha)){
    re[[j]]=sparse.mediation.old(X,M,Y,tol=tol,max.iter=max.iter,lambda = lambda,alpha=alpha[j],figure=NULL,display=FALSE,glmnet.penalty.factor=weights)
    re[[j]]$alpha=alpha[j]
    re[[j]]$tau=tau
  }
  return(re)
